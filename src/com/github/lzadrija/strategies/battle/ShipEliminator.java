 ...
 * 
 *         Used for finding the next target on the grid when in Elimination
 *         mode. This class gets the first hit point and produces next targets
 *         in order to discover all ships that are contained in this area. A
 *         ship can be placed through a hit if it passes only through undefined
 *         cells or other known hits. If a ship can be placed through a cell,
 *         its probability is increased, and most probable cells around a hit
 *         are targeted until no more ships can be positioned trough the hit
 *         area. The result is the list of coordinates that is proven to contain
 *         part of the ship(s).
 */
public class ShipEliminator extends TargetFinder {
	...
	protected Point getNextObjective(Grid grid, ShipsRepository shipsRepository) {
		...
		for (Iterator<Point> iterator = hitsCoordinatesList.iterator(); iterator
				.hasNext();) {
			Point hitPoint = iterator.next();

			// pass through hit coordinate with every undiscovered ship
			for (Iterator<Integer> iterator2 = shipsRepository
					.getHiddenShipsIndexes().iterator(); iterator2.hasNext();) {
				int i = iterator2.next();
				List<Point> shipsAllUpperLeftPoint = shipsRepository
						.getShipsAllPossibleUpperLeftCoordinates(i, hitPoint);

				// check if undiscovered ship can pass through hit on grid in
				// every way possible
				List<Point> shipsStructure = shipsRepository
						.getShipsStructure(i);
				for (Iterator<Point> iterator3 = shipsAllUpperLeftPoint
					...
					if (isGridAvailable) {
						boolean isLikeliHoodIncreased = increaseGridOccupationLikelihood(
								shipsUpperLeftPoint, shipsStructure,
	 ...
	 * @return True if ship can be placed over cells on the grid, false
	 *         otherwise.
	 */
	protected boolean increaseGridOccupationLikelihood(
			Point shipsUpperLeftPoint, List<Point> shipsRelativeCoordinates,
			int[][] cellLikelihoods) {
		boolean isLikeliHoodIncreased = false;
		for (Iterator<Point> iterator = shipsRelativeCoordinates.iterator(); iterator
				.hasNext();) {
			Point relativeCellPosition = iterator.next();

			Point newPoint = shipsUpperLeftPoint
